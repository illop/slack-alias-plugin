...
Bot.prototype.helpList = {
    ...
    update: '*alias update <alias> <list of users>* - This command updates an existing alias with a new list of users. The list of users should be comma-separated and unfortunately can not include the @ symbol.',
...
Bot.prototype.build_pattern = function () {
  ...
  fs.writeFile('./alias_maps.txt', JSON.stringify(self.config.alias_maps), function(err) {
    if(err) {
      ...
      if (msg[0] == 'alias' && msg[1] == 'set') {
        if (msg.length > 4) {
          ...
          if (msg.length > 4) {
              ...
              msgs.push('Sorry, this is not a custom alias.');
          } else if (self.config.alias_maps[msg[2].replace('@','')] == undefined) {
              ...
              self.config.alias_maps[msg[2].replace('@','')] = userlist;
              msgs.push('Alias updated! @' + msg[2].replace('@','') + ' has been set to ' + userlist.join(self.config.link_separator) + '.');
              self.build_pattern();
              if (verbose) {
                  console.log(self.config.alias_maps);
              }
